...
4. ENS/Domain Names (Ethereum)
   ├─ Use human-readable names (vitalik.eth)
   ├─ Verify ENS resolution on multiple sources
   └─ Be aware of similar-looking names
```
...
```
Address Reuse:
├─ DON'T: Reuse addresses (reduces privacy)
├─ DO: Generate new address for each transaction
├─ Bitcoin: HD wallets automatically generate new addresses
└─ Ethereum: Consider using multiple addresses

Public Disclosure:
├─ DON'T: Share wallet addresses publicly
├─ DON'T: Post transaction hashes on social media
├─ DON'T: Discuss holdings publicly
└─ DO: Use separate addresses for public/private transactions

Chain Analysis Resistance:
├─ Use privacy coins (Monero) for sensitive transactions
├─ Use mixing services (CoinJoin for Bitcoin)
├─ Avoid KYC exchanges for privacy-sensitive funds
└─ Use Tor/VPN when accessing wallets

Metadata Protection:
├─ Use VPN or Tor for wallet access
├─ Avoid linking real identity to addresses
├─ Use separate email for crypto accounts
└─ Be cautious with IP address exposure
```
...
```
1. Preparation
   ├─ Set aside dedicated time (no rush)
   ├─ Gather all backup materials
   ├─ Use test wallet or small amount
   └─ Document the process

2. Recovery Execution
   ├─ Wipe test device or use new device
   ├─ Install wallet software (verify official source)
   ├─ Enter seed phrase from backup
   ├─ Verify all accounts/addresses restored
   └─ Check balances match expected

3. Verification
   ├─ Confirm all assets visible
   ├─ Test sending small transaction
   ├─ Verify derivation paths correct
   └─ Document any issues

4. Backup Integrity Check
   ├─ Verify all backup copies readable
   ├─ Check for physical damage (paper, metal)
   ├─ Test encrypted backups (can decrypt)
   └─ Update if necessary
...
**Cryptocurrency Inheritance Strategy**:

```
Option 1: Trusted Executor
├─ Provide seed phrase in sealed envelope
├─ Store with attorney or in safe deposit box
├─ Include instructions for access
└─ Risk: Executor could access early, lose/steal funds

Option 2: Multi-Signature with Timelock
├─ 2-of-3 multisig: You + Executor + Backup
├─ Timelock: Executor can access after inactivity period
├─ Requires technical setup
└─ Benefit: Executor can't access while you're alive

Option 3: Shamir's Secret Sharing
├─ Split seed into shares (e.g., 3-of-5)
├─ Distribute to trusted individuals
├─ No single person can access alone
└─ Benefit: Redundancy and security

Option 4: Professional Custody Service
├─ Services: Casa, Unchained Capital
├─ Inheritance features built-in
├─ Higher cost, less control
└─ Benefit: Professional management

Essential Documentation:
├─ List of all wallets and exchanges
├─ Location of seed phrases/hardware wallets
├─ Instructions for recovery
├─ Contact information for crypto-savvy helper
└─ Store with will/estate documents
```
...
**`/references/wallet-types-security.md`** — Read when selecting a wallet type, comparing hot vs. cold storage options, evaluating custodial vs. non-custodial wallets, setting up hardware wallets, or understanding wallet architecture and security models. Contains detailed comparisons, setup guides, and security analysis for each wallet type.

**`/references/private-key-management.md`** — Read when implementing private key storage solutions, setting up seed phrase backups, configuring multi-signature wallets, implementing Shamir's Secret Sharing, or establishing key rotation procedures. Provides technical implementation details and advanced key management strategies.

**`/references/threat-protection.md`** — Read when defending against phishing attacks, protecting against malware, securing devices, preventing social engineering, or responding to security incidents. Includes attack scenarios, detection methods, and comprehensive defense strategies.

**`/references/recovery-backup.md`** — Read when creating backup procedures, testing wallet recovery, planning for inheritance, implementing disaster recovery, or establishing business continuity for cryptocurrency holdings. Contains step-by-step recovery procedures and estate planning strategies.
